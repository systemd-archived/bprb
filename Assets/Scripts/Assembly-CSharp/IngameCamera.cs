...
public class IngameCamera : WPFMonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			this.m_cameraLimits = default(LevelManager.CameraLimits);
		}
		...
		if (this.m_cameraPreview == null && WPFMonoBehaviour.levelManager != null && WPFMonoBehaviour.levelManager.GoalPosition != null)
		{
			...
			if (WPFMonoBehaviour.levelManager.StartingPosition != null)
			{
			...
			{
				cameraControlPoint3.position = Vector3.zero;
			}
		...
		this.m_cameraBuildZoom = ((this.m_cameraPreview != null && this.m_cameraPreview.ControlPoints.Count > 0) ? this.m_cameraPreview.ControlPoints[this.m_cameraPreview.ControlPoints.Count - 1].zoom : this.m_cameraMinZoom);
	...
	private void UpdatePosition()
	{
		...
		switch (gameState)
		{
			...
			case LevelManager.GameState.Running:
				if (WPFMonoBehaviour.levelManager.ContraptionRunning == null || WPFMonoBehaviour.levelManager.ContraptionRunning.m_cameraTarget == null || GameTime.DeltaTime == 0f)
				{
					return;
				}
	...
	private void AddBirdsToBoundingBox(ref Rect box)
	{
		if (this.m_birdsCache == null)
		{
			...
			{
				this.m_birdsCache = new GameObject[0];
			}
		}
		if (this.m_birdsCache != null)
		{
	...
	{
		if (!this.m_autoZoomEnabled || this.m_returningToDefaultPosition || WPFMonoBehaviour.levelManager == null || WPFMonoBehaviour.levelManager.ContraptionRunning == null)
		{
			return;
		}
