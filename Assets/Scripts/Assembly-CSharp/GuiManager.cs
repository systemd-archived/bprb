...
private Widget RayCast(Vector2 screenPosition)
{
    ...
    // Fix: Check if camera exists before trying to access its gameObject or layer
    if (camera == null)
    {
        return null;
    }
    ...
    {
        // SAFETY CHECK: Ensure focus data and pointers exist before using them
        if (this.m_focusData == null || this.m_focusData.Count == 0 || this.m_pointers == null || this.m_pointers.Count == 0)
        {
            return;
        }

        FocusData focusData = this.m_focusData[0];
        if (focusData == null || this.m_pointers[0] == null)
        {
            return;
        }
        ...
        // This is your original Line 161 - it is now protected by the checks above
        this.PointerInput(0, false, Input.GetMouseButtonDown(0), Input.GetMouseButtonUp(0), Input.GetMouseButton(0), Input.mousePosition, 0, focusData, this.m_pointers[0]);
