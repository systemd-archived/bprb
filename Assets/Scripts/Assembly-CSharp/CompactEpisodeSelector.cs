...
public class CompactEpisodeSelector : MonoBehaviour
{
	...
	private void Awake()
	{
			if (this.m_preButtons != null)
				this.m_preButtons.Clear();

		// Keep main episodes in m_episodes, sandboxes in m_episodesToggled
		this.m_episodes.RemoveAll(ep => ep != null && ep.name != "EpisodeButtonJungle" && ep.name != "EpisodeButtonHalloween");
		if (this.m_episodesToggled == null)
			this.m_episodesToggled = new List<GameObject>();
		this.m_episodesToggled.RemoveAll(ep => ep != null && ep.name != "EpisodeButtonFODSandbox" && ep.name != "EpisodeButtonJungleSandbox");
		...
		if (pointer.down)
		{
			...
			if (Physics.Raycast(this.m_hudCamera.ScreenPointToRay(pointer.position), out raycastHit))
			{
				...
				if (component || (raycastHit.collider.transform.GetComponent<Button>() == null && parent && parent.GetComponent<CompactEpisodeTarget>()))
				{
					...
					{
				    Debug.LogWarning("m_preButtons does not have enough items!");
					}					}
	...
	private void SetCenterEpisode(GameObject target)
	{
		...
		{
			this.m_centerEpisode = ((!this.m_isRotated) ? this.m_preButtons[0] : (this.m_episodesToggled.Count > 0 ? this.m_episodesToggled[0] : null));
		}
		else
		{
	...
	public void MoveToTargetIndex(int index, bool isSandbox = false)
	{
		if (isSandbox)
		{
			...
			if (index == 0 && this.m_preButtons != null && this.m_preButtons.Count > 0)
			{
				this.SetCenterEpisode(this.m_preButtons[0]);
			}
			else
	...
	private void MoveEpisodes(float delta, bool checkCenter = true)
	{
		...
		if (this.m_scrollPivot.transform.localPosition.x > num + this.m_limitMargin && delta > 0f)
		{
			...
			else if (this.CurrentEpisodes.Count > 0)
			{
		...
		{
			if (this.EpisodeCount > 1 && this.CurrentEpisodes.Count > 1)
				this.SetCenterEpisode(this.CurrentEpisodes[this.EpisodeCount - 2]);
			this.m_deltaX = 0f;
		}
		else
		...
		if (checkCenter && this.CurrentEpisodes.Count > 0)
		{
			for (int i = 0; i < this.EpisodeCount; i++)
			{
				...
				if (this.m_centerEpisode == null || num2 < Mathf.Abs(this.m_centerEpisode.transform.position.x))
				{
					...
					if (num3 == 0 && this.m_preButtons != null && this.m_preButtons.Count > 0)
					{
						this.SetCenterEpisode(this.m_preButtons[0]);
					}
					else if (num3 >= 0 && num3 < this.CurrentEpisodes.Count)
					{
	...
	private void Layout()
	{
		...
		if (@bool)
		{
			...
			if (Mathf.Clamp(@int, 0, this.m_episodesToggled.Count - 1) == 0 && this.m_preButtons != null && this.m_preButtons.Count > 0)
			{
				this.SetCenterEpisode(this.m_preButtons[0]);
			}
			else
		...
		{
			this.SetCenterEpisode(this.m_episodes.Count > 0 ? this.m_episodes[0] : null);
		}
	...
	private void OnRotated(bool rotated)
	{
		...
		if (this.EpisodeCount == 0) return;
		if (num2 == 0 && this.m_preButtons != null && this.m_preButtons.Count > 0)
		{
			this.SetCenterEpisode(this.m_preButtons[0]);
		}
		else if (num2 >= 0 && num2 < this.CurrentEpisodes.Count)
		{
